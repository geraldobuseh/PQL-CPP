...
Order order(std::int64_t id, const Symbol& asset, OrderSide side, double size, std::int64_t ms=0) {
    return *Order::create_market(*OrderId::create(id),asset,side,quantity(size),at(ms));
}
Transaction event(PortfolioId id, const Order& request, double price, double fee=0, std::int64_t ms=0) {
...
TEST(Postgres, PriceRoundTripAndMissingLookups) {
    ...
    seed(asset);
    const PriceObservation bar{asset,at(-86400001),"provider's feed",Adjustment::Raw,
        quote(0.1),quote(0.3),quote(0.1),quote(0.2),quantity(0.125)};
    { PostgresUnitOfWork work; work.insertPrice(bar); work.commit(); }
    PostgresUnitOfWork read;
...
TEST(Postgres, SqlFailureRollsBackEarlierWritesAndPoisonsCommit) {
    ...
    EXPECT_EQ(check.exec("SELECT count(*) FROM orders WHERE portfolio_id=$1",pqxx::params{id.value()})[0][0].as<int>(),0);
    EXPECT_EQ(check.exec("SELECT count(*) FROM executions WHERE portfolio_id=$1",pqxx::params{id.value()})[0][0].as<int>(),0);
}
...
TEST(Postgres, DuplicateAndMismatchedRequestsAreRejected) {
    ...
    const auto id=seed(asset);
    const auto request=order(1,asset,OrderSide::Buy,1);
    { PostgresUnitOfWork work; work.append(request,event(id,request,100)); work.commit(); }
    { PostgresUnitOfWork work; EXPECT_THROW(work.append(request,event(id,request,100)),PersistenceError); }
    { PostgresUnitOfWork work; EXPECT_THROW(work.append(order(2,asset,OrderSide::Buy,1),event(id,request,100)),PersistenceError); }
    PostgresUnitOfWork read;
...
TEST(Postgres, CompetingPurchasesSerializeAndCannotOverspend) {
    ...
    auto competing=std::async(std::launch::async,[&] {
        ...
        try { second.append(buy2,event(id,buy2,80)); second.commit(); return true; }
        catch (const PersistenceError&) { return false; }
    });
...
TEST(Postgres, NoncanonicalDatabaseDecimalsFailInsteadOfLosingPrecision) {
    ...
    { pqxx::work tx(connection);
      tx.exec("INSERT INTO market_prices SELECT asset_id,TIMESTAMPTZ 'epoch','external','raw',0.100000000000000001,1,0.1,1,0 FROM assets WHERE symbol='PRECISION'");
      tx.commit(); }
    PostgresUnitOfWork read;
...
TEST(Postgres, ExtremeFiniteDoublesRoundTrip) {
    ...
    const PriceObservation bar{asset,at(1),"synthetic",Adjustment::Raw,quote(tiny),quote(large),quote(tiny),quote(large),quantity(0)};
    { PostgresUnitOfWork work; work.insertPrice(bar); work.commit(); }
    PostgresUnitOfWork read;
...
TEST(Postgres, InvalidOhlcAndUnsupportedTimestampsRollback) {
    ...
    seed(asset);
    const PriceObservation bad{asset,at(1),"synthetic",Adjustment::Raw,quote(100),quote(90),quote(80),quote(100),quantity(0)};
    { PostgresUnitOfWork work;
    ...
    { PostgresUnitOfWork work; EXPECT_THROW(work.insertPrice(outside),PersistenceError); }
}

TEST(Postgres, EmbeddedNullTextRejectsInsteadOfAliasingAndPoisonsWork) {
    const auto asset=symbol("NULLTEXT");
    const auto id=seed(asset);
    const PriceObservation bar{asset,at(0),"feed",Adjustment::Raw,quote(1),quote(1),quote(1),quote(1),quantity(0)};
    { PostgresUnitOfWork work; work.insertPrice(bar); work.commit(); }
    ...
    { PostgresUnitOfWork work;
      ...
      EXPECT_THROW(work.commit(),PersistenceError); }
    { PostgresUnitOfWork work; EXPECT_THROW(work.renamePortfolio(id,hidden),PersistenceError); }
    { PostgresUnitOfWork work; EXPECT_THROW((void)work.createPortfolio(hidden,PortfolioKind::Real,money(1)),PersistenceError); }
    { PostgresUnitOfWork work; EXPECT_THROW(work.addAsset(symbol("NULLNAME"),hidden),PersistenceError); }
    auto invalid_bar=bar;
    invalid_bar.source=hidden;
    { PostgresUnitOfWork work; EXPECT_THROW(work.insertPrice(invalid_bar),PersistenceError); }
...
TEST(Postgres, FailureAfterOrderInsertLeavesNoOrphan) {
    ...
    EXPECT_EQ(check.exec("SELECT count(*) FROM orders WHERE portfolio_id=$1",pqxx::params{id.value()})[0][0].as<int>(),0);
    EXPECT_EQ(check.exec("SELECT count(*) FROM executions WHERE portfolio_id=$1",pqxx::params{id.value()})[0][0].as<int>(),0);
    EXPECT_EQ(check.exec("SELECT count(*) FROM transactions WHERE portfolio_id=$1",pqxx::params{id.value()})[0][0].as<int>(),0);
}
...
TEST(Postgres, ImportedOverflowAndUnderflowFailRead) {
    ...
    { pqxx::work tx(connection);
      for (const auto* value : {"1e400","1e-400","9007199254740993"}) {
          tx.exec("INSERT INTO market_prices SELECT asset_id,TIMESTAMPTZ 'epoch',$1::text,'raw',$1::text::numeric,$1::text::numeric,$1::text::numeric,$1::text::numeric,0 FROM assets WHERE symbol='RANGE'",pqxx::params{value});
      }
      tx.commit(); }
    for (const auto* value : {"1e400","1e-400","9007199254740993"}) {
        PostgresUnitOfWork read;
