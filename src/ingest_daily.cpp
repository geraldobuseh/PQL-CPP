...
int main(int argc,char** argv) {
    ...
    try {
        ...
        const auto calendar=std::chrono::year_month_day{std::chrono::floor<std::chrono::days>(std::chrono::system_clock::now())};
        const auto today=Date::create(int(calendar.year()),unsigned(calendar.month()),unsigned(calendar.day()));
        if(!today) throw MarketDataError("Unsupported current date");
        const auto* key=std::getenv("ALPHAVANTAGE_API_KEY");
        if(!key || !*key) throw MarketDataError("Configure ALPHAVANTAGE_API_KEY in the environment");
        ...
        AlphaVantageProvider provider(transport,key,*today,[](const ProviderAttempt& event) {
            log({{"event",event.retrying?"provider_retry":"provider_request"},
                 {"symbol",event.symbol.value()},{"attempt",event.attempt}});
        });
        unsigned failures=0;
        log({{"event","ingestion_start"},{"from",formatMarketDate(from)},{"to",formatMarketDate(to)},
             {"symbols",symbols.size()},{"fetch_only",fetch_only}});
        for(const auto& symbol:symbols) {
            const auto start=std::chrono::steady_clock::now();
            try {
                const auto bars=provider.getHistory(symbol,from,to);
                if(fetch_only) {
                    log({{"event","fetch_complete"},{"symbol",symbol.value()},{"fetched",bars.size()}});
                    continue;
                }
                ...
                log({{"event","symbol_committed"},{"symbol",symbol.value()},{"fetched",bars.size()},
                    {"inserted",counts.inserted},{"unchanged",counts.unchanged},{"elapsed_ms",elapsed}});
                ...
                ++failures;
                log({{"event","symbol_failed"},{"symbol",symbol.value()},
                    {"quality_code",issueName(error.issue())},{"field",fieldName(error.field())},
                    {"session_date",error.date()?nlohmann::json(formatMarketDate(*error.date())):nlohmann::json(nullptr)}});
            } catch(const MarketDataError& error) {
                ++failures;
                log({{"event","symbol_failed"},{"symbol",symbol.value()},{"reason",error.what()}});
