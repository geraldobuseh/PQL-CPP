...
std::optional<Date> Date::create(int year, unsigned month, unsigned day) {
    ...
    const std::chrono::year_month_day value{std::chrono::year{year}, std::chrono::month{month},
                                            std::chrono::day{day}};
    if (!value.ok()) return std::nullopt;
    return Date{value};
}

PriceBar::PriceBar(const Symbol& symbol, Date date, Price open, Price high, Price low, Price close,
                   Quantity volume)
    : symbol_(symbol),
      date_(date),
      open_(open),
...
std::optional<PriceBar> PriceBar::create(const Symbol& symbol, Date date, Price open, Price high,
                                         Price low, Price close, Quantity volume) {
    if (low.value() > high.value() || open.value() < low.value() || open.value() > high.value() ||
