...
namespace {
struct CurlRuntime {
    CurlRuntime() { if(curl_global_init(CURL_GLOBAL_DEFAULT)!=CURLE_OK) throw MarketDataError("HTTP initialization failed"); }
    ~CurlRuntime() { curl_global_cleanup(); }
};
struct Buffer { std::string body; std::optional<unsigned> retry; bool failed{false}; };
std::size_t receive(char* data,std::size_t size,std::size_t count,void* context) noexcept {
    ...
    if(bytes>2*1024*1024-buffer.body.size()) { buffer.failed=true; return 0; }
    try { buffer.body.append(data,bytes); return bytes; }
    catch(...) { buffer.failed=true; return 0; }
}
std::size_t header(char* data,std::size_t size,std::size_t count,void* context) noexcept {
    ...
    try {
        ...
        if(colon!=std::string::npos) {
            ...
            std::transform(name.begin(),name.end(),name.begin(),[](unsigned char c){return char(c>='A'&&c<='Z'?c+('a'-'A'):c);});
            if(name=="retry-after") {
                const auto start=line.find_first_not_of(" \t",colon+1);
                if(start==std::string::npos) buffer.retry=31;
                else {
                    ...
                    buffer.retry=parsed.ec==std::errc{} && parsed.ptr==value.data()+value.size()?seconds:31;
                }
            }
        }
        return bytes;
...
HttpResponse CurlHttpTransport::get(const std::string& url) {
    if(!url.starts_with("https://www.alphavantage.co/query?") || url.find('\0')!=std::string::npos)
    ...
    Buffer buffer;
    auto check=[](CURLcode code) { if(code!=CURLE_OK) throw MarketDataError("HTTP configuration failed"); };
    ...
    if(buffer.failed) throw MarketDataError("Provider response exceeded limits or could not be buffered");
    if(code==CURLE_OPERATION_TIMEDOUT || code==CURLE_COULDNT_CONNECT || code==CURLE_COULDNT_RESOLVE_HOST ||
       code==CURLE_RECV_ERROR || code==CURLE_SEND_ERROR || code==CURLE_PARTIAL_FILE)
