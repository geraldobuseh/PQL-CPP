...
[[noreturn]] void malformed() {
    throw MarketDataValidationError(MarketDataIssue::MalformedResponse);
}
...
double number(const nlohmann::json& field) {
    ...
    if (exponent != std::string::npos) {
        ...
        if (point <= 0)
            serialized = "0." + std::string(static_cast<std::size_t>(-point), '0') + digits;
...
Price readPrice(const nlohmann::json& row, const char* key, MarketDataField field, Date date) {
    if (!row.contains(key) || row.at(key).is_null() ||
        (row.at(key).is_string() &&
         row.at(key).get<std::string>().find_first_not_of(" \t\r\n") == std::string::npos))
        throw MarketDataValidationError(MarketDataIssue::MissingPrice, field, date);
    std::optional<Price> price;
    try {
        price = Price::create(number(row.at(key)));
...
Quantity readVolume(const nlohmann::json& row, Date date) {
    ...
    try {
        ...
        throw MarketDataValidationError(MarketDataIssue::InvalidVolume, MarketDataField::Volume,
                                        date);
    }
...
const std::vector<Symbol>& supportedMarketSymbols() {
    static const std::vector<Symbol> symbols{*Symbol::create("SPY"),   *Symbol::create("QQQ"),
                                             *Symbol::create("AAPL"),  *Symbol::create("MSFT"),
                                             *Symbol::create("NVDA"),  *Symbol::create("AMZN"),
                                             *Symbol::create("GOOGL"), *Symbol::create("META")};
    return symbols;
}
Date parseMarketDate(const std::string& text) {
    const auto invalid = [] {
        throw MarketDataValidationError(MarketDataIssue::InvalidDate, MarketDataField::SessionDate);
    };
    ...
    const auto date = Date::create(std::stoi(text.substr(0, 4)),
                                   static_cast<unsigned>(std::stoi(text.substr(5, 2))),
...
std::string formatMarketDate(Date date) {
    ...
    out << std::setfill('0') << std::setw(4) << int(value.year()) << '-' << std::setw(2)
        << unsigned(value.month()) << '-' << std::setw(2) << unsigned(value.day());
    return out.str();
}

std::vector<PriceBar> parseAlphaVantageDaily(const std::string& response, const Symbol& symbol,
                                             Date today) {
    supported(symbol);
    ...
    try {
        ...
        const auto data = nlohmann::json::parse(
            response, [&](int depth, nlohmann::json::parse_event_t event, nlohmann::json& value) {
                ...
                if (event == nlohmann::json::parse_event_t::key) {
                    ...
                    if (!keys.back().names.insert(pending_key).second) {
                        if (keys.back().daily)
                            throw MarketDataValidationError(MarketDataIssue::DuplicateDate,
                                                            MarketDataField::SessionDate,
                                                            parseMarketDate(pending_key));
                        malformed();
                    }
                }
        ...
        if (data.contains("Note") || data.contains("Information"))
            throw MarketDataError(
                "Alpha Vantage quota or entitlement response; retry after checking provider "
                "limits");
        if (data.contains("Error Message"))
            throw MarketDataError("Alpha Vantage rejected the request");
        const auto& meta = data.at("Meta Data");
        if (meta.at("2. Symbol") != symbol.value() || meta.at("5. Time Zone") != "US/Eastern")
            malformed();
        const auto refreshed = [&] {
            try {
                return parseMarketDate(meta.at("3. Last Refreshed").get<std::string>());
            } catch (const MarketDataValidationError&) {
                throw MarketDataValidationError(MarketDataIssue::InvalidDate,
                                                MarketDataField::LastRefreshed);
            }
        }();
        if (refreshed > today)
            throw MarketDataValidationError(MarketDataIssue::FutureTimestamp,
                                            MarketDataField::LastRefreshed, refreshed);
        ...
        for (const auto& [key, row] : series.items()) {
            ...
            if (date > today)
                throw MarketDataValidationError(MarketDataIssue::FutureTimestamp,
                                                MarketDataField::SessionDate, date);
            newest = date;
            ...
            if (!bar) {
                const auto field = high.value() < low.value()
                                       ? MarketDataField::High
                                       : (open.value() < low.value() || open.value() > high.value()
                                              ? MarketDataField::Open
                                              : MarketDataField::Close);
                throw MarketDataValidationError(MarketDataIssue::InvalidOhlc, field, date);
            }
            if (date < today)
                bars.push_back(*bar);  // Never ingest a potentially open current session.
        }
...
std::vector<PriceBar> AlphaVantageProvider::fetch(const Symbol& symbol) {
    ...
    const auto url =
        ...
        "query?function=TIME_SERIES_DAILY&outputsize=compact&datatype=json&symbol=" +
        ...
        try {
            ...
            if (response.status == 429 || response.status == 408 || response.status >= 500) {
                if (response.retry_after_seconds) {
                    if (*response.retry_after_seconds > 30)
                        throw MarketDataError(
                            "Provider retry delay exceeds this run's limit; rerun later");
...
Price AlphaVantageProvider::getLatestPrice(const Symbol& symbol) {
    return fetch(symbol).back().close();
}
std::vector<PriceBar> AlphaVantageProvider::getHistory(const Symbol& symbol, Date from, Date to) {
    ...
    for (const auto& bar : bars)
        if (bar.date() >= from && bar.date() <= to) result.push_back(bar);
    return result;
}
}  // namespace pql
